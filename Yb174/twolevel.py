...
class TwoLevelSystem:
    ...
    def H(self):
        ...
        laser_field = 2 * np.pi * self.delta * 10 ** 6 * self.basis[1] * self.basis[1].dag()

        off_diagonal_elements = [self.omega[i][j] / 2 * self.basis[i] * self.basis[j].dag() * cg[i][j] ** 2
                                 for i, j in itertools.product(range(2), range(2))]
        return laser_field + sum(off_diagonal_elements)
    ...
    def raw_decay(self):
        ...
        decay[1] = [np.sqrt(2 * np.pi * self.linewidth) * cg[0][1] * self.basis[0] * self.basis[1].dag()]
        return decay
    ...
    def omega(self):
        """Rabi frequencies."""
        omega = np.zeros((2, 2))
        omega[1][0] = omega[0][1] = 2 * np.pi * self.linewidth * np.sqrt(self.sat / 2)
        return omega
